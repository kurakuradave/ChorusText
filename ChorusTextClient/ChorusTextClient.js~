...
var gnome = require( './routes/gnome' );
gnome.setCD( cd );
...
//=============================================================
// socket.io application logic (RUNNING AS CLIENT TO WEBSERVER)
//=============================================================
// socket.io stuff
var ioClient = require( 'socket.io-client' );
var socketClient = ioClient.connect(fullServerAddress, {reconnect: true});

socketClient.on('cnected2ssn', function( data ) { 
     console.log( "ChorusText device connected to server via socket! " ); 
     console.log( data.msg );
});

socketClient.on( 'newUserJoined', function( data ) {  
    console.log( "A new user just joined: " + data.userID );
} );
...
// =================================================================
// socket.io application logic (RUNNING AS SERVER FOR OTHER DEVICES)
// =================================================================
                    ...
                    if( obj.hasOwnProperty( 't' ) ) { // turn
                        ...
                        switch( cFocus ) {
                            ...
                            case 1 :
                                cs.sys_say( "location" );
                                ccdocuID = "testdocu"
                                // make HTTP request to download content of the document
                                var daURL = fullServerAddress + '/ccd/' + ccdocuID;
                                var  daqs = { 'userID' : userID, 'ccdocuID' : ccdocuID, 'devNum' : devNum };
                                reqq.get( { url: daURL, form: daqs }, function (err, httpResponse, body) {
                                    console.log( body );
                                    // emit to session that content is loaded and ready to receive updates
                                    socketClient.emit( 'cnect2ssn', { "ccdocuID" : "testdocu", "userID" : "CT0079" } );
                                } );
                            break;
                            case 2 :
                                cs.sys_say( "main" );
